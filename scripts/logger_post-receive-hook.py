...
if os.environ['REQUEST_METHOD'] != 'POST':
	log_message = '[ERROR {}] Invalid HTTP request. Only POST requests accepted'.format(call_time)
...
#    log_message = '[ERROR {}] Invalid request. Missing JSON payload'.format(call_time)
	...
	if getattr(args, 'file'):
		try:
			...
			payload = simplejson.loads(args.file.read())
            ...
            log_message = '[ERROR {}] Invalid POST request. No payload'.format(call_time)
